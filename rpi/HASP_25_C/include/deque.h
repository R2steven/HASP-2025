...
typedef struct {
    ...
    uint8_t dataSize; //size of dataMembers
    size16_t queueSize; //size of underlying data array
    size16_t size; // current size of deque
 ...
 */
Deque *initDequeCalloc(uint16_t queueSize, uint8_t sizeOfData);

/**
 * allocates required memory for dequeue construction and initilizes the 
 * resulting data structure.
 * 
 * allows the passing of a user defined calloc function, must take as input parameter
 * the size of memory to allocate in bytes and return a pointer to the allocated
 * memory. the allocated memory must be initialized to 0. 
 * 
 ...
 * @param sizeOfData the size of each data object stored in queue
 */
Deque *initDeque(void *(*myCallocFunc)(size_t,size_t), void (*myFreeFunc)(void*),
                    size_t queueSize, uint8_t sizeOfData);

/**
 * perform all necessary operations to destroy the deque. 
 */
 ...
 */
bool8_t insertRight(Deque *deque, uchar *item);
 ...
 *          BE SAME DATA SIZE AS deque->dataSize
 */
bool8_t dqPush(Deque *deque, void *item);
 ...
 */
bool8_t dqPop(Deque *deque, void *ret);
 ...
 *          BE SAME DATA SIZE AS deque->dataSize
 */
bool8_t dqEnqueue(Deque *deque, void *item);
 ...
 *          MUST BE SAME DATA SIZE AS deque->dataSize
 */
bool8_t dqDequeue(Deque *deque, void *ret);
